...
public class DespesasDAO {
	...
	public List<Despesas> getDespesasAPagar(Date dataInicial, Date dataFinal) {
		try {
			...
			Query query = em.createQuery(
					"SELECT d FROM Despesas d WHERE d.situacao = 'A PAGAR' and d.vencimento between :dataInicial and :dataFinal and d.usuariosId.id = :id ORDER BY d.vencimento, d.descricao");
	...
	public List<Despesas> getDespesasPagas(Date dataInicial, Date dataFinal) {
		try {
			Query query = em.createQuery(
					"SELECT d FROM Despesas d WHERE d.situacao = 'PAGO' and d.vencimento between :dataInicial and :dataFinal and d.usuariosId.id = :id ORDER BY d.vencimento, d.descricao");
			query.setParameter("dataInicial", dataInicial);
			query.setParameter("dataFinal", dataFinal);
			query.setParameter("id", Integer.parseInt(MainScreenFXMLController.getId()));
			return query.getResultList();
	...
	public List<Despesas> getDespesasPorVencimento(Date vencimento, String situacao) {
		try {
			@SuppressWarnings("JPQLValidation")
			Query query = em.createQuery(
					"SELECT d FROM Despesas d WHERE d.situacao = :situacao and d.vencimento = :vencimento and d.usuariosId.id = :id ORDER BY d.vencimento, d.descricao");
	...
	public List<Despesas> getDespesasVencidas() {
		try {
			@SuppressWarnings("JPQLValidation")
			Query query = em.createQuery(
					"SELECT d FROM Despesas d WHERE date(d.vencimento) < CURRENT_DATE() and situacao = 'A PAGAR' and d.usuariosId.id = :id order by vencimento, d.descricao");
			query.setParameter("id", Integer.parseInt(MainScreenFXMLController.getId()));
			return query.getResultList();
		} catch (NumberFormatException e) {
			Alerts.showAlert("Controleasy", null, e.getMessage(), Alert.AlertType.ERROR);
	...
	public BigDecimal getTotalDespesasAPagar(Date dataInicial, Date dataFinal) {
		try {
			...
			TypedQuery<BigDecimal> query = em.createQuery(
					"SELECT sum(d.valor) FROM Despesas d WHERE d.situacao = 'A PAGAR' and d.vencimento between :dataInicial and :dataFinal and d.usuariosId.id = :id",
					BigDecimal.class);
			query.setParameter("dataInicial", dataInicial);
			query.setParameter("dataFinal", dataFinal);
	...
	public BigDecimal getTotalDespesasPagas(Date dataInicial, Date dataFinal) {
		try {
			@SuppressWarnings("JPQLValidation")
			TypedQuery<BigDecimal> query = em.createQuery(
					"SELECT sum(d.valor) FROM Despesas d WHERE d.situacao = 'PAGO' and d.vencimento between :dataInicial and :dataFinal and d.usuariosId.id = :id",
					BigDecimal.class);
			query.setParameter("dataInicial", dataInicial);
			query.setParameter("dataFinal", dataFinal);
			query.setParameter("id", Integer.parseInt(MainScreenFXMLController.getId())).getSingleResult();
	...
	public BigDecimal getDespesasTotais(Date dataInicial, Date dataFinal) {
		try {
			@SuppressWarnings("JPQLValidation")
			TypedQuery<BigDecimal> query = em.createQuery(
					"SELECT sum(d.valor) FROM Despesas d WHERE d.vencimento between :dataInicial and :dataFinal and d.usuariosId.id = :id",
					BigDecimal.class);
			query.setParameter("dataInicial", dataInicial);
			query.setParameter("dataFinal", dataFinal);
			query.setParameter("id", Integer.parseInt(MainScreenFXMLController.getId())).getSingleResult();
	...
	public boolean atualizarDespesa(Despesas d) {
		try {
			...
			if (d.getSituacao().equals("PAGO") && despesas.getPagamento() == null) {
				despesas.setPagamento(new Date());
			}
			if (d.getSituacao().equals("A PAGAR")) {
				despesas.setPagamento(null);
			}
	...
	public List<Despesas> getGroupCategorias(Date dataInicial, Date dataFinal) {
		...
		try {
			...
			TypedQuery<Object[]> query = em.createQuery(
					"SELECT d.categoria as categoria, SUM(d.valor) as valor FROM Despesas d WHERE d.vencimento between :dataInicial and :dataFinal and d.usuariosId.id = :id GROUP BY d.categoria",
					Object[].class);
			query.setParameter("dataInicial", dataInicial);
			query.setParameter("dataFinal", dataFinal);
			query.setParameter("id", Integer.parseInt(MainScreenFXMLController.getId()));
