...
import br.com.controleasy.dao.RelatorioPorCategoriaDAO;
 ...
 * @author luanp
 */
public class RelatorioPorCategoriaFXMLController implements Initializable {
        ...
        if (this.getDataInicial().getValue() != null && this.getDataFinal().getValue() != null) {
            ...
            if (this.getRadioButtonPagamento().isSelected()) {
                if (this.getRadioButtonTodos().isSelected()) {
                    relatorio = "DespesasPorPagamentoPorCategoria";
                    RelatorioPorCategoriaDAO.gerarRelatorioPorCategoria(dateInicial, dateFinal, relatorio, image);
                }
            }
            if (this.getRadioButtonVencimento().isSelected()) {
                if (this.getRadioButtonAPagar().isSelected()) {
                    situacao = "A pagar";
                    relatorio = "DespesasPorVencimentoPorSituacaoPorCategoria";
                    RelatorioPorCategoriaDAO.gerarRelatorioPorCategoriaPorSituacao(dateInicial, dateFinal, situacao, relatorio, image);
                } else if (this.getRadioButtonPagas().isSelected()) {
                    situacao = "Pago";
                    relatorio = "DespesasPorVencimentoPorSituacaoPorCategoria";
                    RelatorioPorCategoriaDAO.gerarRelatorioPorCategoriaPorSituacao(dateInicial, dateFinal, situacao, relatorio, image);
                } else if (this.getRadioButtonTodos().isSelected()) {
                    relatorio = "DespesasPorVencimentoPorCategoria";
                    RelatorioPorCategoriaDAO.gerarRelatorioPorCategoria(dateInicial, dateFinal, relatorio, image);
                }
            }
        } else {
