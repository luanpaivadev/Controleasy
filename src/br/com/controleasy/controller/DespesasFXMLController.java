...
public class DespesasFXMLController implements Initializable {
					...
					if (result == sim) {
						try {
							Parent parent = FXMLLoader
									.load(getClass().getResource("/br/com/controleasy/view/DespesasFXML.fxml"));
							Image image = new Image(
									getClass().getResourceAsStream("/br/com/controleasy/images/favicon.png"));
	...
	public void getDespesasAPagar() {
		Double total = 0.00;
		List<Despesas> list = new DespesasDAO().getDespesasAPagar(DespesasFXMLController.getDateInicial(),
				DespesasFXMLController.getDateFinal());
		for (Despesas d : list) {
	...
	public void getDespesasPagas() {
		Double total = 0.00;
		List<Despesas> list = new DespesasDAO().getDespesasPagas(DespesasFXMLController.getDateInicial(),
				DespesasFXMLController.getDateFinal());
		for (Despesas d : list) {
			total += d.getValor().doubleValue();
		}
		...
		try {
			...
			if (this.getFiltroVencimento().getValue() != null) {
				...
				Date vencimento = Date
						.from(this.getFiltroVencimento().getValue().atStartOfDay(ZoneId.systemDefault()).toInstant());
	...
	void buttonCadastrarDespesa(ActionEvent event) {
		...
		try {
			if (this.getTextFieldDescricao().getText().equals("") && this.getTextFieldValor().getText().equals("")
					&& this.getVencimentoDespesa().getValue() == null) {
				Alerts.showAlert("Controleasy", "(*) CAMPOS OBRIGATÓRIOS",
						"PREENCHA OS CAMPOS DE DESCRIÇÃO, VALOR E VENCIMENTO", Alert.AlertType.ERROR);
			} else if (this.getTextFieldDescricao().getText().equals("")) {
				Alerts.showAlert("Controleasy", "(*) CAMPO OBRIGATÓRIO", "PREENCHA O CAMPO DE DESCRIÇÃO",
						Alert.AlertType.ERROR);
			} else if (this.getTextFieldValor().getText().equals("")) {
				Alerts.showAlert("Controleasy", "(*) CAMPO OBRIGATÓRIO", "PREENCHA O CAMPO DE VALOR",
						Alert.AlertType.ERROR);
			} else if (this.getVencimentoDespesa().getValue() == null) {
				Alerts.showAlert("Controleasy", "(*) CAMPO OBRIGATÓRIO", "PREENCHA O CAMPO DE VENCIMENTO",
						Alert.AlertType.ERROR);
			} else {
				...
				if (new DespesasDAO().cadastrarDespesa(despesa)) {
					Alerts.showAlert("Controleasy", null, "DESPESA CADASTRADA COM SUCESSO!",
			...
			Alerts.showAlert("Controleasy", "VALOR INVÁLIDO!", "PREENCHA SOMENTE NÚMEROS NO CAMPO VALOR",
					Alert.AlertType.ERROR);
		}
				...
				alert.showAndWait().ifPresent(result -> {
					if (result == sim) {
						...
						despesa.setCategoria(
								this.getCategoriaDespesa().getSelectionModel().getSelectedItem().toString());
						despesa.setValor(new BigDecimal(this.getTextFieldValor().getText().replace(",", ".")));
						LocalDate localDate = this.getVencimentoDespesa().getValue();
						Date date = Date.from(localDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
						despesa.setVencimento(date);
						...
						if (new DespesasDAO().atualizarDespesa(despesa)) {
							Alerts.showAlert("Controleasy", null, "DESPESA ATUALIZADA COM SUCESSO!",
									Alert.AlertType.INFORMATION);
							this.getDespesasAPagar();
							this.getTextFieldId().setText("");
							this.getTextFieldDescricao().setText("");
				...
				alert.showAndWait().ifPresent(result -> {
					if (result == sim) {
						...
						if (new DespesasDAO().deletarDespesa(despesa)) {
							Alerts.showAlert("Controleasy", null, "DESPESA DELETADA COM SUCESSO!",
									Alert.AlertType.INFORMATION);
							this.getDespesasAPagar();
							this.getTextFieldId().setText("");
							this.getTextFieldDescricao().setText("");
				...
				alert.showAndWait().ifPresent(result -> {
					if (result == sim) {
						...
						if (new DespesasDAO().arquivarDespesa(despesa)) {
							Alerts.showAlert("Controleasy", null, "DESPESA ARQUIVADA COM SUCESSO!",
									Alert.AlertType.INFORMATION);
							this.getDespesasAPagar();
							this.getTextFieldId().setText("");
							this.getTextFieldDescricao().setText("");
	...
	void setDespesaFormulario(MouseEvent event) {
		try {
			Despesas despesa = this.getTableViewDespesas().getItems()
					.get(this.getTableViewDespesas().getSelectionModel().getSelectedIndex());
